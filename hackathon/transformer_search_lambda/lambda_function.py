...
def lambda_handler(event, context):
    """
    This is the Lambda handler function. It receives an event object 
    and a context object when invoked.

    Args:
        event (dict): Event data passed to the function. Should contain the 'query' key.
        context: Lambda context information.

    Returns:
        dict: A dictionary containing the response from your function.
    """
    ...
    if not query:
        return {
            'statusCode': 400,
            'body': json.dumps({'error': 'Missing "query" parameter in the request.'})
        }
